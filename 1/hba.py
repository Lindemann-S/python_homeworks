...
def rabin_karp(sub_str, str):
    result = 0
    str_len = len(str)
    sub_str_len = len(sub_str)
    sub_str_hash = phash(sub_str)
    for i in range(str_len - sub_str_len + 1):
        str_hash = phash(str[i:i+sub_str_len])
        if str_hash == sub_str_hash and str[i:i+sub_str_len] == sub_str:
            result += 1
    return result
...
def kmers(seq, k):
    ...
    for i in range(len(seq) - k + 1):
        sub_str = seq[i:i+k]
        dict[sub_str] = rabin_karp(sub_str, seq)
    #print(dict)
    return dict
    ...
    if distance == hamming:
        for line1, index1 in zip(lines[:max_index - 1], range(max_index - 1)):
            for line2, index2 in zip(lines[index1 + 1:], range(index1 + 1, max_index)):
                dict[index1 * 10 + index2] = hamming(line1, line2)

    if distance == distance1:
        for line1, index1 in zip(lines[:max_index - 1], range(max_index - 1)):
            for line2, index2 in zip(lines[index1 + 1:], range(index1 + 1, max_index)):
                dict[index1 * 10 + index2] = distance1(line1, line2)
